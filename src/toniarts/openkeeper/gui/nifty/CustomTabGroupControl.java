...
public class CustomTabGroupControl extends AbstractController implements TabGroup {
     ...
     */
    private static final Logger log = System.getLogger(CustomTabGroupControl.class.getName());
    ...
    public void bind(Nifty nifty, Screen screen, Element elmnt, Parameters prmtrs) {
        ...
        if (tabButtonPanel == null) {
            log.log(Level.ERROR, "Panel for the tabs not found. Tab group will not work properly. Looked for: #tab-button-panel");
        } else {
            final Element buttonElement = tabButtonPanel.findElementById("#button-template");
            if (buttonElement == null) {
                log.log(Level.ERROR, "No template for tab button found. Tab group will be unable to display tabs. Looked for: "
                        + "#button-template");
            } else {
        ...
        if (contentPanel == null) {
            log.log(Level.ERROR, "Content panel not found. Tab group will be unable to display tab content. Looked for: "
                    + "#tab-content-pane");
        }
    }
    ...
    private void initTab(final Tab tab) {
        ...
        if (button == null) {
            if (buttonTemplate == null || nifty == null || screen == null || tabButtonPanel == null) {
                log.log(Level.ERROR, "Tab can't be initialized. Binding not done yet or binding failed.");
                return;
            }
    ...
    private void checkVisibility() {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (button == null) {
                log.log(Level.WARNING, "Something is wrong with the tabs. Tab button not there anymore.");
                continue;
            }
    ...
    public void onStartScreen() {
        if (nifty == null || screen == null) {
            log.log(Level.ERROR, "Starting screen failed. Seems the binding is not done yet.");
        }
        String id = getId();
        if (id != null) {
    ...
    public void setSelectedTabIndex(final int index) {
        ...
        if (nifty != null) {
            ...
            if (id != null) {
                ...
                if (tab == null) {
                    log.log(Level.ERROR, "Tab with valid index returned null. This looks like a internal error.");
                } else {
