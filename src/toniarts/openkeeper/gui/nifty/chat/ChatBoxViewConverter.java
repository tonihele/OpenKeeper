...
public class ChatBoxViewConverter implements ListBoxViewConverter<ChatEntry> {
    ...
    private static final Logger log = System.getLogger(ChatBoxViewConverter.class.getName());
    ...
    public final void display(final Element listBoxItem, final ChatEntry item) {
        ...
        if (text == null) {
            log.log(Level.ERROR, "Failed to locate text part of chat line! Can't display entry.");
        }
        final TextRenderer textRenderer = text.getRenderer(TextRenderer.class);
        if (textRenderer == null) {
            log.log(Level.ERROR, "Text entry of the chat line does not contain the required text renderer.");
            return;
        }
